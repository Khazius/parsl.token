...
{
  //eosio_assert(owner == N(seedtutorial), "staking currently disabled");
  require_auth(owner);
  auto sym = quantity.symbol;
  eosio_assert(sym.is_valid(), "invalid symbol name");
  ...
  stats statstable( _self, sym.name() );
  const auto& st = statstable.get( sym.name(), "symbol does not exist" );
  eosio_assert( st.supply.symbol == sym, "symbol precision mismatch" );
...
{
  //eosio_assert(owner == N(seedtutorial), "unstaking currently disabled");
  require_auth(owner);
  auto sym = quantity.symbol;
  eosio_assert(sym.is_valid(), "invalid symbol name");
  eosio_assert(quantity.amount > 0, "must specify positive quantity");
  ...
  out.actions.emplace_back(permission_level{_self, N(refund)},
    _self, N(refund),
